import { buildShopping } from "../public/shopping.js";

export { buildShopping };
...
export function buildSwapPrompt({ offers, dislikes, persons, diet, day, avoid, keepOfferIds }) {
  return [
    ...
    `Ernährungsform: ${diet}.`,
    dislikeLine(dislikes),
    avoid.length ? `Das neue Gericht muss sich deutlich von diesen Gerichten unterscheiden (andere Hauptzutat, andere Küche): ${avoid.join("; ")}.` : "",
    keepOfferIds.length
...
export async function requestSwap({ offers, dislikes, persons, diet, day, avoid = [], keepOfferIds = [] }, { fetchImpl = fetch } = {}) {
  ...
  const input = await callClaude(
    SWAP_TOOL,
    buildSwapPrompt({ offers: limited, dislikes, persons, diet, day, avoid, keepOfferIds: keepOfferIds.filter((id) => known.has(id)) }),
    fetchImpl,
  );
  ...
  return {
    ...
    ingredients: day.ingredients.map((ing) => {
      ...
      return {
        ...
        amount: ing.amount,
        offer: offer
          ? { id: offer.id, retailer: offer.retailer, retailerId: offer.retailerId, product: offer.product, price: offer.price, validTo: offer.validTo }
          : null,
      };
    }),
  };
